...
TGOutput TankGauge::calculate_all(TGInput& input) {
    TGOutput result;
    ...
    result.k_val = calc_k_val(result.t_val, input.choice_15_20);
    result.ctl_val = calc_CTL_val(result.beta_val, result.t_val, input.choice_15_20);
    result.d_oil_val = calc_d_oil(input.denst_calc_val, result.k_val, result.ctl_val);
    result.s_avg_val = calc_s_avg(result.v_val_interp, input.alpha, result.t_val, input.level_calc_val);
    ...
    result.k_koef_val = calc_k_koef(input.v_tabl_val, input.level_calc_val, result.v_val_interp);
